...
fn child(config: ContainerOpts) -> isize {
    match setup_container_configurations(&config){
        Ok(_) => log::info!("Container set up seccessfully"),
        Err(e) => {
    ...
    log::info!("Startin container with command {} and args {:?}", config.path.to_str().unwrap(), config.argv);
